...
async function patchReportById(reportId, payload, baseUrl = "") {
  const response = await fetch(
    buildApiUrl(`/api/reports/${encodeURIComponent(reportId)}`, baseUrl),
    {
      method: "PATCH",
  ...
  if (!response.ok) {
    const message = await getErrorMessage(response, "Failed to update report.");
    throw new Error(message);
  }
...
export async function assignTeamToReport(reportId, assignedTeamId, actorRole, baseUrl = "") {
  if (!assignedTeamId || typeof assignedTeamId !== "string") {
    throw new Error("A valid team ID is required.");
  }

  if (!actorRole || typeof actorRole !== "string") {
    throw new Error("A valid actor role is required.");
  }

  return patchReportById(reportId, { assignedTeamId, actorRole }, baseUrl);
}

export async function updateReportStatus(reportId, status, actorRole, baseUrl = "") {
  ...
  return patchReportById(reportId, { status, actorRole }, baseUrl);
}

export async function createActivity(activity, baseUrl = "") {
  const response = await fetch(buildApiUrl("/api/activities", baseUrl), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    const message = await getErrorMessage(response, "Failed to create activity.");
    throw new Error(message);
  }
